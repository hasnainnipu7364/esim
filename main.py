...
def show_countries(call):
    ...
    markup.add(InlineKeyboardButton("🔙 Back to Continents", callback_data="back_to_continents"))
...
def back_to_continents(call):
    ...
    for cont in continents:
        markup.add(InlineKeyboardButton(cont, callback_data=f"continent_{cont}"))
    bot.edit_message_text("🌍 Choose a continent:", chat_id=call.message.chat.id,
                          message_id=call.message.message_id, reply_markup=markup)
...
def show_plans_paginated(call):
    ...
    back_markup.add(InlineKeyboardButton("🔙 Back to Countries", callback_data=f"continent_{get_continent_by_country(country)}"))
